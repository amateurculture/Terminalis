...
namespace Controllers
{
	...
	public class ThirdPersonPlayer : MonoBehaviour
	{
		...
		[SerializeField] float m_GroundCheckDistance = .5f;
        ...
        public ThirdPersonPlayer character { get; private set; }
        ...
        void Start()
        {
            ...
            character = GetComponent<ThirdPersonPlayer>();
		...
		public void Move(Vector3 move, bool crouch, bool jump)
		{
			...
			if (m_IsGrounded)
			{
                // This enables jumping
				HandleGroundedMovement(crouch, jump);
			}
			else
			{
                // This handles resolving recovery from jumping... and damn is it broken
		...
		void PreventStandingInLowHeadroom()
		{
			...
			if (!m_Crouching && m_Rigidbody != null && m_Capsule != null)
			{
				Ray crouchRay = new Ray(m_Rigidbody.position + Vector3.up * m_Capsule.radius * k_Half, Vector3.up);
				float crouchRayLength = m_CapsuleHeight - m_Capsule.radius * k_Half;
				if (Physics.SphereCast(crouchRay, m_Capsule.radius * k_Half, crouchRayLength, Physics.AllLayers, QueryTriggerInteraction.Ignore))
		...
		void UpdateAnimator(Vector3 move)
		{
            ...
            // Required for setting speed, but is not responsible for motion itself
            if (player == gameObject)
                m_Animator.SetFloat("Forward", m_ForwardAmount, 0.1f, Time.deltaTime);
            else
                m_Animator.SetFloat("Forward", m_ForwardAmount * 1.8f, 0.1f, Time.deltaTime);
			...
			{
                // Removal of this line will get you stuck in mid air during a jumps
		...
		public void OnAnimatorMove()
		{
			...
			if (m_IsGrounded && Time.deltaTime > 0 && m_Animator != null)
			{
...
#endif
            Debug.DrawRay(transform.position + (Vector3.up * .25f), Vector3.down * 1.5f, Color.blue);

            // 0.1f is a small offset to start the ray from inside the character
            // it is also good to note that the transform position in the sample assets is at the base of the character

            //if (Physics.Raycast(transform.position + (Vector3.up * 0.1f), Vector3.down, out hitInfo, m_GroundCheckDistance))
            if (Physics.SphereCast(transform.position + (Vector3.up * .75f), .5f, Vector3.down, out hitInfo, 1.5f, layerMask))
            {
